...
const FormOgloszenia = () => {
    ...
    const ogloszenie = () => {
        let jsx = [];
        const onChange = index => e => setOgloszenia(prevState => ({
            ...prevState,
            [index]: e.target.value
        }))

        for (let i = 0; i < count; i++) {
            jsx.push(<input key={i} type="text" placeholder={"Ogloszenie " + i} value={ogloszenia[i]}
                onChange={onChange(i)}>
            </input>)
        }
        return (
    ...
    return (
        <div className='form-ogloszenia'>
            ...
            <form onSubmit={onSubmit} className='form-ogloszenia-form'>
                ...
                <input type="text" placeholder='data' value={date} onChange={((e) => setDate(e.target.value))} />
                {ogloszenie().map((item) =>
                    item)}
